...
@media (max-width: 768px) {
    ...
    /* Mejora para checkboxes en sidebar */
...
# =========================
# MAIN APP - FILTROS CON CHECKBOXES
# =========================
def main():
    # Cargar datos
    ...
    # =========================
    # FILTROS CON CHECKBOXES (como en la captura)
    # =========================
    ...
    # Checkbox para rango de años
    show_year_filter = st.sidebar.checkbox("📅 Rango de años", value=True)
    if show_year_filter:
    ...
    # Checkbox para Open Access
    show_oa_filter = st.sidebar.checkbox("🔓 Open Access", value=True)
    if show_oa_filter:
        oa_filter = st.sidebar.radio("Open Access", ["Todos", "Solo OA", "No OA"])
    
    # Checkbox para Cuartiles
    show_quart_filter = st.sidebar.checkbox("📊 Cuartiles", value=True)
    if show_quart_filter:
    ...
    # Checkbox para Departamentos
    show_dept_filter = st.sidebar.checkbox("🏥 Departamentos", value=True)
    if show_dept_filter:
        dept_filter = st.sidebar.multiselect("Departamentos", sorted(df["Departamento"].astype(str).unique()), default=None)
    
    # Checkbox para Búsqueda
    show_search_filter = st.sidebar.checkbox("🔍 Búsqueda", value=True)
    ...
    # =========================
    # Módulo de carga y merge (al final del sidebar)
    # =========================
    st.sidebar.markdown("---")
    st.sidebar.header("🔄 Actualizar dataset")
    
    new_files = st.sidebar.file_uploader("Nuevos CSV/XLSX", type=["csv","xlsx"], accept_multiple_files=True)
    btn_prev = st.sidebar.button("👀 Previsualizar unión")
    btn_apply = st.sidebar.button("✅ Aplicar actualización", type="primary")
    ...
    if not new_df.empty and btn_prev:
        # Función para construir match key
