# /app/app.py
# Dashboard Cienciométrico — primera hoja XLSX, merge/dedup, WordCloud, cuartiles, y keys únicas en charts
...
# -----------------------------
# Deduplicación y lectura de nuevos archivos (igual que antes)
# -----------------------------
...
def _fig_quartile_by_year(dff: pd.DataFrame):
    tmp = dff.copy()
    ...
    piv = (tmp.pivot_table(index="_Year", columns="Quartile", values="Title", aggfunc="count", fill_value=0)
              ...
              .sort_index())
    fig = px.bar(piv, x=piv.index, y=piv.columns, title="Cuartiles por año (apilado)")
    return fig

# -----------------------------
# Tabs (agregamos 🏷️ Cuartiles)
# -----------------------------
...
with tabs[0]:
    ...
    # Pequeña tarjeta de composición por cuartil
    ...
    if pdf_bytes:
        st.download_button("⬇️ PDF — Reporte resumido", pdf_bytes, "reporte_dashboard.pdf", "application/pdf", key="dl_pdf_summary")
    else:
...
with tabs[5]:
    ...
    if "Open Access" in dff.columns and len(dff):
        fig_oa_tab = _fig_oa_pie(dff)  # fig nuevo para evitar reuso
    ...
    if "Quartile" in dff.columns and dff["Quartile"].notna().any():
        ...
        with c2:
            if dff["_Year"].notna().any():
                fig_q_year = _fig_quartile_by_year(dff)
                st.plotly_chart(fig_q_year, use_container_width=True, key="chart_quartile_year")
                png2 = _plotly_png(fig_q_year)
                if png2:
                    st.download_button("⬇️ PNG — Cuartiles por año", png2, "cuartiles_por_anio.png", "image/png", key="dl_quartile_year")
        # Tabla
