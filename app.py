# /app/app.py
# Dashboard Cienciométrico — fix DuplicateElementId (keys únicas), cuartiles con filtro y gráficos, WordCloud, JIF/SJR
...
CAND = {
    ...
    "quartile": ["Quartile", "Quartil", "Cuartil", "JCR_Quartile", "SJR_BestQuartile", "BestQuartile_combined", "best_quartile", "Quartile_clean"],
}

# -----------------------------
# Helpers de UI (keys únicas)
# -----------------------------
def ukey(prefix: str) -> str:
    """Por qué: evitar StreamlitDuplicateElementId sin depender de orden/estado."""
    return f"{prefix}_{uuid4().hex}"

def show_plot(fig, name: str):
    """Wrapper para plotly con key única por render."""
...
def _norm_text(s: object) -> str:
    if pd.isna(s): return ""
    t = str(s).strip(); return re.sub(r"\s+", " ", t)
...
def wordcloud_png(freq: Dict[str, int], width: int = 1600, height: int = 900) -> Optional[bytes]:
    try:
        ...
        wc = WordCloud(width=width, height=height, background_color="white", collocations=False,
                       normalize_plurals=False, prefer_horizontal=0.9, random_state=42)
...
def _num_like(x: object) -> float:
    ...
    m = re.search(r"[-+]?\d+(?:\.\d+)?", s); return float(m.group(0)) if m else np.nan
...
def unify_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if jif_cols: out["JIF"] = out[jif_cols].applymap(_num_like).bfill(axis=1).iloc[:,0]
    else: out["JIF"] = pd.to_numeric(out.get("JIF"), errors="coerce")
    sjr_cols = [c for c in out.columns if re.search(r"\bsjr\b", c, re.I)]
    if sjr_cols: out["SJR"] = out[sjr_cols].applymap(_num_like).bfill(axis=1).iloc[:,0]
    else: out["SJR"] = pd.to_numeric(out.get("SJR"), errors="coerce")
    return out

# -----------------------------
# Normalización base + cuartil + OA
# -----------------------------
def normalize_dataset(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if qcol:
        ...
        df["Quartile"] = q.str.extract(r"(Q[1-4])", expand=False).fillna("Sin cuartil")
    else:
...
if new_files:
    ...
    if not new_df.empty:
        ...
        if btn_apply:
            ...
            st.session_state["__df_updated__"] = merged; df = merged
            xbytes = _df_to_xlsx_bytes(df)
            if xbytes: dl_button("⬇️ Descargar dataset ACTUALIZADO (XLSX)", xbytes, "dataset_actualizado.xlsx",
...
def kpis_summary(dff: pd.DataFrame) -> Dict[str, str]:
    return {
        ...
        "Mediana JIF": f"{pd.to_numeric(dff.get('JIF'), errors='coerce').median():.2f}" if "JIF" in dff.columns and dff["JIF"].notna().any() else "—",
...
def quartile_pivot(dff: pd.DataFrame) -> pd.DataFrame:
    ...
    tmp["_Year"] = tmp["_Year"].astype(int); tmp["Quartile"] = tmp["Quartile"].fillna("Sin cuartil")
    piv = (tmp.pivot_table(index="_Year", columns="Quartile", values="Title", aggfunc="count", fill_value=0)
             .reindex(columns=["Q1","Q2","Q3","Q4","Sin cuartil"], fill_value=0).sort_index())
    return piv
...
# RESUMEN
with tabs[0]:
    k1, k2, k3, k4, k5 = st.columns(5); KP = kpis_summary(dff)
    k1.metric("Nº publicaciones", KP["Nº publicaciones"]); k2.metric("% con DOI", KP["% con DOI"])
    k3.metric("% OA", KP["% OA"]); k4.metric("Mediana JIF", KP["Mediana JIF"]); k5.metric("% Q1", KP["% Q1"])
...
with tabs[4]:
    ...
    if "Title" not in df.columns or df["Title"].dropna().empty:
        ...
        with c4: stop_extra = st.text_input("Stopwords extra (coma separadas)", "study,analysis,clinical,trial,case,report,review,role,effect", key="titles_stop_extra")
...
with tabs[5]:
    ...
    if "Open Access" in dff.columns and len(dff):
        fig = fig_oa_pie(dff)        # fig NUEVO por tab (no reutilizado)
        show_plot(fig, "oa_tab")
        png = _plotly_png(fig)
    ...
    if "Quartile" in dff.columns and dff["Quartile"].notna().any():
        ...
        with c2:
            if dff["_Year"].notna().any():
                ...
                dl_button("⬇️ CSV — Tabla año×cuartil", df_long.to_csv(index=False).encode("utf-8"), "cuartiles_por_anio.csv", "text/csv", "dl_quartile_csv")
